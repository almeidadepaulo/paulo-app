...
<cffunction 
    ...
    <cfargument name="bodyErrors" type="struct" required="true">

    <cfloop collection="#bodyErrors#" item="key">
        
        <cfscript>
            var error = bodyErrors[key];
            var label = error.label;
            var value = "";

            // verificar se o campo foi definido
            // aceita vazio ('')
            if (structKeyExists(error, "defined") AND error.defined AND NOT structKeyExists(body, key)) {
                
                throw (message = "Campo " & label & " não foi definido" , errorCode = "400");

            } else if(structKeyExists(body, key)) {

                value = body[key];    

            }

            // campo requiredo? não aceita ''                    
            if (structKeyExists(error, "required") // existe propriedade required ? 
                AND error.required // required === true
                AND (NOT structKeyExists(body, key) OR trim(value) EQ "") // campo não existe no body ou está vazio
            ){

                throw (message = "Campo " & label & " obrigatório" , errorCode = "400");

            } else if (NOT structKeyExists(body, key)) { // se o campo não for obrigatório e não for passado, desconsiderar
                continue;                
            }       
                       
            if (structKeyExists(error, "type")) { // verificar type
                
                if(error.type EQ "numeric" AND not IsNumeric(value)) {
                    
                    throw (message = "Campo " & label & " precisa ser numérico" , errorCode = "400");

                } else if(error.type EQ "date" AND not isDate(value)) {
                    
                     throw (message = "Campo " & label & " precisa ser uma data" , errorCode = "400");

                }
            }                   

            if (structKeyExists(error, "max") AND value GT error.max) { 
             
                throw (message = label & ": Maior que " & error.max , errorCode = "400");
    
            }   

            if (structKeyExists(error, "maxLength") AND len(value) GT error.maxLength) {
             
                throw (message = label & ": Quantidade de caracteres mairo que " & error.maxLength , errorCode = "400");
