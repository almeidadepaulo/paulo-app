(function() {
    ...
    angular
        .module('seeawayApp')
        .factory('extratoService', extratoService);

    extratoService.$inject = ['config', '$http', '$q', 'toastService', '$timeout'];

    function extratoService(config, $http, $q, toastService, $timeout) {
        var service = {};
        ...
        function get(params) {
            ...
            var req = $http({
                    url: config.REST_URL + '/collect/extrato',
                    method: 'GET',
                    headers: {
                        'Authorization': '',
