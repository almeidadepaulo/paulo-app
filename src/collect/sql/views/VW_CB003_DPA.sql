...
-- Apaga view
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VW_CB003_DPA]'))
DROP VIEW [dbo].[VW_CB003_DPA]
GO

-- PUBLISH - Publicações -  Distribuição por publicação anual
	
-- Cria View
CREATE VIEW [dbo].[VW_CB003_DPA]
WITH ENCRYPTION
AS
...
SELECT
  ...
  CB003_CD_PUBLIC,
  CB060_DS_PUBLIC,
  FLOOR(CB003_DT_MOVTO / 10000) CB003_DT_MOVTO,
  ...
  SUM((CB003_TT_CUSPOS) + (CB003_TT_CUSPUB) + (CB003_TT_CUSCAP) + (CB003_TT_CUSCAP)) CB003_VL_TOT
FROM
  CB003,
  CB060,
  CB053
...
-- AND (@ENT_DT_ANO  IS NULL OR FLOOR(CB003_DT_MOVTO / 10000) = @ENT_DT_ANO)
/* CB003 -> CB060 */
