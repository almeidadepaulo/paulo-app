...
-- Apaga view
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[VW_CB003_DIST_DIARIO_REGIAO]'))
DROP VIEW [dbo].[VW_CB003_DIST_DIARIO_REGIAO]
GO

-- PUBLISH - Publicações -  Distribuição por região diária
	
-- Cria View
CREATE VIEW [dbo].[VW_CB003_DIST_DIARIO_REGIAO]
WITH ENCRYPTION
AS
...
SELECT
  ...
  SUM((CB003_TT_CUSINS) + (CB003_TT_CUSCAP) + (CB003_TT_CUSPOS) + (CB003_TT_CUSPUB)) CB003_VL_TOT
FROM
  CB003,
  CB060,
  CB053,
