(function() {
    ...
    angular.module('seeawayApp').controller('ConciliadorOcorrenciaCtrl', ConciliadorOcorrenciaCtrl);

    ConciliadorOcorrenciaCtrl.$inject = ['config', 'exampleService', '$rootScope', '$scope', '$state', '$mdDialog'];

    function ConciliadorOcorrenciaCtrl(config, exampleService, $rootScope, $scope, $state, $mdDialog) {
        ...
        vm.dgExemploConfig = {
            url: config.REST_URL + '/contabil/conciliador-ocorrencia',
            method: 'GET',
            fields: [{
                title: 'Conta SICLID',
                field: 'COL_1',
        ...
        function dgExemploLabel(event) {
            ...
            var n = Math.abs((numeral().unformat(event.data.value.COL_10)).toFixed(2));
            /* jshint ignore:start */
            if (event.item.label === 'diferenca' && n !== 0) {
                //console.info('n', n);

                if (n === 34.36) {
                    event.data.COL_10 = '<div class="diferenca">' + event.data.COL_10 +
        ...
        function itemClick(event) {
            ...
            /*else if (event.itemClick.diferenca) {}*/
        }
